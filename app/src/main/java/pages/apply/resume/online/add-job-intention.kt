...
open class GenPagesApplyResumeOnlineAddJobIntention : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesApplyResumeOnlineAddJobIntention) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesApplyResumeOnlineAddJobIntention
            ...
            val pageState = reactive<PageState>(PageState(isEditMode = false, submitting = false, showPositionPicker = false, showCityPicker = false, showIndustryPicker = false, positionName = "", industryName = "", selectedIndustryCode = ""))
            ...
            val positionState = reactive<PositionState>(PositionState(categoryList = _uA<GetAllPositionResult>(), categoryLoading = false, currentCategoryIndex = 0, searchKeyword = "", searchResultList = _uA<GetPositionResult>(), searchPageNum = 1, searchHasMore = true, searchLoading = false, childLoading = false))
            val cityState = reactive<CityState>(CityState(provinceList = _uA<GetAddressesResult>(), cityList = _uA<GetAddressesResult>(), currentProvinceIndex = 0, provinceLoading = false, cityLoading = false))
            val industryState = reactive<IndustryState>(IndustryState(list = _uA<GetAllPositionResult>(), currentParentIndex = 0, loading = false))
            val pageTitle = computed(fun(): String {
                return if (pageState.isEditMode) {
                    "修改求职意向"
                } else {
                    "添加求职意向"
                }
            ...
            val handleIndustrySelect = fun(item: GetAllPositionResult){
                ...
                val parent = industryState.list[industryState.currentParentIndex]
                val parentName = if (parent != null) {
                    asText(parent.PositionName)
                } else {
                    ""
                }
            ...
            val handleSubmit = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        if (!hasText(pageState.positionName)) {
                            return@w1 uni_showToast(ShowToastOptions(title = "请选择意向职位", icon = "none"))
                        }
            ...
            val handleDelete = fun(){
                uni_showModal(ShowModalOptions(title = "提示", content = "确定要删除这条求职意向吗？", confirmColor = "#0d9488", success = fun(res){
                    if (!res.confirm) {
                        return
                    }
                    confirmDelete()
            ...
            return fun(): Any? {
                return _cE("view", _uM("class" to "flex flex-col h-full bg-white"), _uA(
                    _cV(unref(GenComponentsNavbarIndexClass), _uM("title" to " ")),
                    _cE("view", _uM("class" to "flex-1 flex flex-col px-6 pt-4"), _uA(
                        _cE("view", _uM("class" to "mb-8"), _uA(
                            _cE("text", _uM("class" to "text-2xl font-bold text-gray-900 mb-2 block"), _tD(pageTitle.value), 1),
                            _cE("text", _uM("class" to "text-sm text-gray-500"), "不同的求职意向会获得不同的精选职位")
                        )),
                        _cE("view", _uM("class" to "flex flex-col"), _uA(
                            _cE("view", _uM("class" to "flex flex-row items-center justify-between py-6 border-b border-gray-100", "onClick" to handlePositionClick), _uA(
                                _cE("text", _uM("class" to "text-base text-gray-600"), "意向职位"),
                                _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                                    if (isTrue(hasText(pageState.positionName))) {
                                        _cE("text", _uM("key" to 0, "class" to "text-base text-gray-900 font-medium"), _tD(pageState.positionName), 1)
                                    } else {
                                        _cE("text", _uM("key" to 1, "class" to "text-base text-gray-400"), "请选择")
                                    }
                                    ,
                            ...
                            _cE("view", _uM("class" to "flex flex-row items-center justify-between py-6 border-b border-gray-100", "onClick" to handleCityClick), _uA(
                                _cE("text", _uM("class" to "text-base text-gray-600"), "工作城市"),
                                _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                                    if (isTrue(hasText(formData.CityName))) {
                                        _cE("text", _uM("key" to 0, "class" to "text-base text-gray-900 font-medium"), _tD(formData.CityName), 1)
                                    } else {
                                        _cE("text", _uM("key" to 1, "class" to "text-base text-gray-400"), "请选择")
                                    }
                                    ,
                            ...
                            _cE("view", _uM("class" to "flex flex-row items-center py-6 border-b border-gray-100"), _uA(
                                _cE("text", _uM("class" to "text-base text-gray-600 mr-4"), "薪资要求"),
                            ...
                            _cE("view", _uM("class" to "flex flex-row items-center justify-between py-6 border-b border-gray-100", "onClick" to handleIndustryClick), _uA(
                                _cE("text", _uM("class" to "text-base text-gray-600"), "期望行业"),
                                _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                                    if (isTrue(hasText(pageState.industryName))) {
                                        _cE("text", _uM("key" to 0, "class" to "text-base text-gray-900 font-medium"), _tD(pageState.industryName), 1)
                                    } else {
                                        _cE("text", _uM("key" to 1, "class" to "text-base text-gray-400"), "请选择")
                                    }
                                    ,
                    ...
                    _cE("view", _uM("class" to "p-6 pb-10"), _uA(
                        if (isTrue(pageState.isEditMode)) {
                            _cE("view", _uM("key" to 0, "class" to "flex flex-row gap-4"), _uA(
                                ...
                                _cE("view", _uM("class" to "flex-1"), _uA(
                                    _cV(unref(GenUniModulesTangUiXComponentsTButtonIndexClass), _uM("type" to "primary", "text" to "保存", "size" to "large", "shape" to "round", "loading" to pageState.submitting, "custom-class" to "t-default", "style" to _nS(_uM("backgroundColor" to "#0d9488", "borderColor" to "#0d9488")), "onClick" to handleSubmit), null, 8, _uA(
                                        "loading",
                                        "style"
                                    ))
                                ))
                            ...
                            _cV(unref(GenUniModulesTangUiXComponentsTButtonIndexClass), _uM("key" to 1, "type" to "primary", "text" to "保存", "size" to "large", "shape" to "round", "block" to "", "loading" to pageState.submitting, "custom-class" to "t-default", "style" to _nS(_uM("backgroundColor" to "#0d9488", "borderColor" to "#0d9488")), "onClick" to handleSubmit), null, 8, _uA(
                                "loading",
                                "style"
                            ))
                        }
                            ...
                            _cE("view", _uM("class" to "flex flex-col h-full bg-white"), _uA(
                                ...
                                _cE("view", _uM("class" to "flex-1 flex flex-row overflow-hidden"), _uA(
                                    ...
                                    _cE("scroll-view", _uM("scroll-y" to "", "class" to "flex-1 bg-white h-full p-4"), _uA(
                                        ...
                                        if (currentSubIndustryList.value.length === 0) {
                                            _cE("view", _uM("key" to 0, "class" to "flex items-center justify-center py-10"), _uA(
                                                _cE("text", _uM("class" to "text-gray-400 text-sm"), "暂无二级数据")
                                            ))
                                        } else {
                                            _cC("v-if", true)
                                        }
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("t-radio-group__item" to _pS(_uM("flexDirection" to "row-reverse", "justifyContent" to "space-between", "paddingTop" to 12, "paddingRight" to 16, "paddingBottom" to 12, "paddingLeft" to 16)))
            }
