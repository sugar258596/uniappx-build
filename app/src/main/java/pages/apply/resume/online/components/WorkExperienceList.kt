...
open class GenPagesApplyResumeOnlineComponentsWorkExperienceList : VueComponent {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {}
    open var list: UTSArray<WorkExperienceItem> by `$props`
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesApplyResumeOnlineComponentsWorkExperienceList) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesApplyResumeOnlineComponentsWorkExperienceList
            val _cache = __ins.renderCache
            val resumeStore = useResumeStore()
            val handleAdd = fun(){
                uni_navigateTo(NavigateToOptions(url = "/pages/apply/resume/online/add-work-experience"))
            }
            val handleEdit = fun(index: Number){
                val raw = (resumeStore.resumeData.WorkExperienceCodeData ?: _uA())[index]
                if (raw != null) {
                    resumeStore.setEditingWorkExperience(raw)
                }
                val id = raw?.WorkExperienceId ?: 0
                uni_navigateTo(NavigateToOptions(url = "/pages/apply/resume/online/add-work-experience?id=" + id))
            }
            return fun(): Any? {
                return _cE("view", _uM("class" to "bg-white rounded-lg p-6"), _uA(
                    _cE("view", _uM("class" to "flex flex-row items-center justify-between mb-4"), _uA(
                        _cE("text", _uM("class" to "text-base font-bold text-gray-800"), "工作经历"),
                    ...
                    _cE("view", _uM("class" to "flex flex-col"), _uA(
                        _cE(Fragment, null, RenderHelpers.renderList(_ctx.list, fun(exp, index, __index, _cached): Any {
                            return _cE("view", _uM("key" to index, "class" to _nC(_uA(
                                ...
                                _cE("view", _uM("class" to "flex-1 flex flex-col gap-2"), _uA(
                                    _cE("view", _uM("class" to "flex flex-row items-center justify-between"), _uA(
                                        _cE("text", _uM("class" to "text-base text-gray-800"), _tD(exp.CompanyName), 1),
                                        _cV(unref(GenUniModulesTangUiXComponentsTIconIndexClass), _uM("name" to "icon-youjiantou", "size" to 28, "color" to "#ccc"))
                                    )),
                                    _cE("text", _uM("class" to "text-sm text-gray-500"), _tD(exp.JobBeginTime) + "-" + _tD(exp.JobEndTime) + " " + _tD(exp.JobName), 1),
                                    _cE("text", _uM("class" to "text-sm text-gray-600 leading-relaxed"), _tD(exp.ContentDetail), 1)
                                ))
                            ), 10, _uA(
                                "onClick"
                            ))
