...
open class GenPagesApplyResumeOnlineAddProjectExperience : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesApplyResumeOnlineAddProjectExperience) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesApplyResumeOnlineAddProjectExperience
            ...
            val formData = reactive<AddEditProjectExperienceParams>(AddEditProjectExperienceParams(ProjectName = "", ProjectPositionName = "", ContentDetail = "", JobBeginTime = "", JobEndTime = ""))
            val showInputPicker = ref(false)
            val inputType = ref<InputType__1>("ProjectName")
            ...
            val inputPopupTitle = computed(fun(): String {
                if (inputType.value === "ProjectName") {
                    return "项目名称"
                }
                if (inputType.value === "ProjectPositionName") {
                    return "项目角色"
                }
                return "项目描述"
            }
            ...
            val getInputValue = fun(type: InputType__1): String {
                if (type === "ProjectName") {
                    return formData.ProjectName
                }
                if (type === "ProjectPositionName") {
                    return formData.ProjectPositionName
                }
                return formData.ContentDetail
            }
            val setInputValue = fun(type: InputType__1, value: String){
                if (type === "ProjectName") {
                    formData.ProjectName = value
                } else if (type === "ProjectPositionName") {
                    formData.ProjectPositionName = value
                } else {
            ...
            val handleInputClick = fun(type: InputType__1){
            ...
            val handleSubmit = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        if (!hasText(formData.ProjectName)) {
                            uni_showToast(ShowToastOptions(title = "请输入项目名称", icon = "none"))
                            return@w1
                        }
                        if (!hasText(formData.ProjectPositionName)) {
                            uni_showToast(ShowToastOptions(title = "请输入项目角色", icon = "none"))
                            return@w1
                        }
                        if (!hasText(formData.JobBeginTime)) {
                            uni_showToast(ShowToastOptions(title = "请选择项目时间段", icon = "none"))
                            return@w1
                        }
                        if (!hasText(formData.ContentDetail)) {
                            uni_showToast(ShowToastOptions(title = "请输入项目描述", icon = "none"))
                            return@w1
                        }
                        submitting.value = true
                        try {
                            await(addEditProjectExperience(formData))
            ...
            val confirmDelete = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        try {
                            await(deleteProjectExperience(DeleteProjectExperienceParams(WorkExperienceId = formData.WorkExperienceId!!)))
            ...
            val handleDelete = fun(){
                uni_showModal(ShowModalOptions(title = "提示", content = "确定要删除这条项目经历吗？", confirmColor = "#0d9488", success = fun(res){
                    if (res.confirm) {
                        confirmDelete()
                    }
                }
            ...
            onLoad(fun(options: UTSJSONObject){
                ...
                if (idStr != null) {
                    ...
                    if (id > 0) {
                        ...
                        val editing = resumeStore.editingProjectExperience
                        formData.ProjectName = editing.ProjectName ?: ""
                        formData.ProjectPositionName = editing.ProjectPositionName ?: ""
                ...
                return _cE("view", _uM("class" to "flex flex-col h-full bg-white"), _uA(
                    ...
                    _cE("view", _uM("class" to "flex-1 flex flex-col gap-4 px-4"), _uA(
                        _cE("view", _uM("class" to "flex flex-row items-center justify-between p-4 border-b border-gray-100", "onClick" to fun(){
                            handleInputClick("ProjectName")
                        }
                        ), _uA(
                            _cE("text", _uM("class" to "text-sm text-gray-800"), "项目名称"),
                            _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                                if (isTrue(!hasText(formData.ProjectName))) {
                                    _cE("text", _uM("key" to 0, "class" to "text-sm text-gray-400"), "请输入")
                                } else {
                                    _cE("text", _uM("key" to 1, "class" to "text-sm text-gray-800"), _tD(formData.ProjectName), 1)
                                }
                                ,
                        ...
                        _cE("view", _uM("class" to "flex flex-row items-center justify-between p-4 border-b border-gray-100", "onClick" to fun(){
                            handleInputClick("ProjectPositionName")
                        }
                        ), _uA(
                            _cE("text", _uM("class" to "text-sm text-gray-800"), "项目角色"),
                            _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                                if (isTrue(!hasText(formData.ProjectPositionName))) {
                                    _cE("text", _uM("key" to 0, "class" to "text-sm text-gray-400"), "请选择")
                                } else {
                                    _cE("text", _uM("key" to 1, "class" to "text-sm text-gray-800"), _tD(formData.ProjectPositionName), 1)
                                }
                                ,
                                _cV(unref(GenUniModulesTangUiXComponentsTIconIndexClass), _uM("name" to "icon-youjiantou", "size" to 32, "color" to "#999"))
                            ))
                        ...
                        _cE("view", _uM("class" to "flex flex-row items-center justify-between p-4 border-b border-gray-100", "onClick" to handleTimeClick), _uA(
                            _cE("text", _uM("class" to "text-sm text-gray-800"), "项目时间段"),
                            _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
                        ...
                        _cE("view", _uM("class" to "flex flex-row items-center justify-between p-4 border-b border-gray-100", "onClick" to fun(){
                            ...
                            _cE("text", _uM("class" to "text-sm text-gray-800"), "项目描述"),
                            _cE("view", _uM("class" to "flex flex-row items-center gap-2"), _uA(
